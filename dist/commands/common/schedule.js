...
module.exports = class ScheduleCommand extends (discord_js_commando_1.Command) {
    constructor(client) {
        super(client, {
            name: 'schedule',
            aliases: ['schedules'],
            ...
            args: [
                {
                    ...
                    type: 'string',
                    validate: (text) => schedules.hasOwnProperty(text)
                },
            ],
        });
    }
    run(message, { day }) {
        if (schedules.hasOwnProperty(day))
            return message.say(formatSchedule(schedules[day]));
